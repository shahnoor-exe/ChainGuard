...
export default function ShipmentTable({ shipments = [], loading, onViewDetail }) {
  ...
  if (loading) return (
    <div className="card">
      {Array.from({ length: 10 }).map((_, i) => (
        <div key={i} className="flex gap-4 p-4" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
  ...
  return (
    <div className="card">
      {/* Filters */}
      <div className="flex flex-wrap gap-3 p-4" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
        ...
        <span className="text-xs self-center" style={{ color: 'var(--text-secondary)' }}>{sorted.length} results</span>
      </div>
        ...
        <table className="w-full text-sm">
            ...
            <tr style={{ borderBottom: '1px solid var(--border-subtle)' }}>
              {COLS.map(col => (
                <th key={col.key}
                  ...
                  className={`px-4 py-3 text-left text-xs font-medium uppercase tracking-wider ${col.sortable ? 'cursor-pointer select-none' : ''}`}
          ...
          <tbody>
            {paged.length === 0 ? (
              ...
              <tr key={s.id}
                className={`transition-colors hover:bg-[var(--bg-elevated)] ${s.status === 'delivered' ? 'opacity-60' : ''}`}
                style={{
                  borderBottom: '1px solid var(--border-subtle)',
                  borderLeft: s.status === 'at_risk' ? '2px solid var(--status-danger)' : s.status === 'delayed' ? '2px solid var(--status-warn)' : 'none',
                }}>
                <td className="px-4 py-3">
                  <span className="font-mono text-xs font-medium" style={{ color: 'var(--accent-primary)' }}>{s.shipment_code}</span>
                </td>
                <td className="px-4 py-3">
                  <span className="text-xs" style={{ color: 'var(--text-primary)' }}>{s.origin_city}</span>
                  <span className="mx-1" style={{ color: 'var(--text-faint)' }}>→</span>
                  <span className="text-xs" style={{ color: 'var(--text-primary)' }}>{s.destination_city}</span>
                </td>
                <td className="px-4 py-3 text-xs" style={{ color: 'var(--text-secondary)' }}>{s.carrier_name}</td>
